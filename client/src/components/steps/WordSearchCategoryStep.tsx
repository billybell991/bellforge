...
import type { WordSearchCategoryOption } from '../../types';
import { WORDSEARCH_CATEGORIES } from '../../types';

interface WordSearchCategoryStepProps {
  selected: WordSearchCategoryOption | null;
  onSelect: (s: WordSearchCategoryOption) => void;
}

export function WordSearchCategoryStep({ selected, onSelect }: WordSearchCategoryStepProps) {
  ...
  return (
    <div>
      <div className="step-header">
        ...
        <p className="step-subtitle">What kind of words should be hidden in your grid?</p>
      </div>

      <div className="options-grid">
        {WORDSEARCH_CATEGORIES.map((s) => (
          <div
            key={s.id}
      ...
      {selected?.id === 'custom' && (
        <div className="custom-input-area" style={{ marginTop: '1.5rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <input
            ...
            placeholder="e.g. Types of dinosaurs, Harry Potter spells..."
            autoFocus
            style={{
